...
## Темы форума и авто-дайджест

В **супергруппе с темами** создайте тему (например «Сводка дня») и дайте боту право **публиковать сообщения** в темах.

Расписание задаётся **командами в чате** (без отдельных переменных в `.env`), тем же пользователям, что и `/digest` — в `DIGEST_ALLOWED_USER_IDS`:

- `/autostart 9:00` — каждый день в **09:00 по `DIGEST_TZ`** публиковать дайджест **как `/digest` без аргументов** (календарный **вчера**, по **всем** сообщениям группы в БД, не только из темы). Если команду написать **внутри темы**, публикация пойдёт **в эту тему**; из «Общего» чата — в общий топик.
- `/autostart_info` — текущее расписание для этой группы.
- `/autostart_stop` — отключить автозапуск для этой группы.

Настройки хранятся в SQLite (таблица `digest_autostart`). Срабатывание по расписанию **не проверяет** `DIGEST_ALLOWED_USER_IDS` (это действие бота); настройку расписания — только разрешённые пользователи.
